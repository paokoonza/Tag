...
˘„˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘˘
"""
helpMessage2 ="""?®≈§·??? Œ≤«æ≈¶ ?´·??ê·û∂‡Æ§‡?‡™Ø‡??ì‡∫Æ‡∏µ‚ú®
...
omikuzi = ["Â§ßÂ?","‰∏≠Â?","Â∞èÂ?","?´Â?","Â§ßÂá∂","??1?7"]

wait = {
    'contact':False,
    'autoJoin':True,
    'autoCancel':{"off":False,"members":50},
    'leaveRoom':False,
    'timeline':False,
    'autoAdd':False,
    'message':"‡πÅ‡∏≠‡∏î‡∏°‡∏≤‡∏£‡∏±‡?‡∏´‡∏£‡∏∑‡∏≠‡πÅ‡∏≠‡∏î‡∏°‡∏≤‡∏£‡∏±‡?‡∏Ñ‡∏£‡∏±‡?",
    "lang":"JP",
    "comment":"Auto like By.?®≈¶Œ?Ñ‚Ñ≥???´·??ê·û∂‡Æ§‡?‡™Ø‡??ì‡∫Æ‡∏µ‚ú® ",
    "likeOn":False,
    "commentOn":False,
    "commentBlack":{},
    "wblack":False,
    ...
    try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        kk.sendText(op.param1,"Wekawekaweka ????Har Har?øø")
        ...
        if op.type == 26:
            ...
            if msg.contentType == 26:
                ...
                msg.text = None
                cl.sendMessage(msg)
            elif msg.text in ["?õ„ÅÆ?ó„É¨?º„É≥??1?7","K1 gift"]:
                msg.contentType = 9
                ...
                msg.text = None
                ki.sendMessage(msg)
            elif msg.text in ["?õ„ÅÆ?ó„É¨?º„É≥??1?7","K2 gift"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '6'}
                msg.text = None
                kk.sendMessage(msg)
            elif msg.text in ["?õ„ÅÆ?ó„É¨?º„É≥??1?7","K3 gift"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '3'}
                msg.text = None
                ks.sendMessage(msg)
            elif msg.text in ["?õ„ÅÆ?ó„É¨?º„É≥??1?7","K4 gift"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '3'}
                msg.text = None
                kc.sendMessage(msg)
            elif msg.text in ["?õ„ÅÆ?ó„É¨?º„É≥??1?7","K5 gift"]:
                msg.contentType = 9
                msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '3'}
                ...
                if c in [""," ","\n",None]:
                    ...
                    wait["comment"] = c
                    cl.sendText(msg.to,"It was changed?Ç\n\n" + c)
            elif msg.text in ["Comment check"]:
                cl.sendText(msg.to,"An automatic comment is established as follows at present?Ç\n\n" + str(wait["comment"]))
            elif msg.text in ["?≥„É°?≥„?:?™„É≥","Comment:on"]:
            ...
            elif msg.text in ["?≥„É°?≥„?:?™„?","Comment:off"]:
                ...
                if msg.from_ in Administrator:
                    ...
                    cl.sendText(msg.to,"?‘º‘º????")
                else:
                    cl.sendText(msg.to,"?‘º∆¶???∆≥")
                ...
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        if ginfo.preventJoinByTicket == True:
                            ...
                            u = "Ë®±ÂèØ"
                        cl.sendText(msg.to,"[?∆¶?∆≤∆§]\n" + str(ginfo.name) + "\n\n[???]\n" + msg.to + "\n\n[?∆¶?∆≤∆§ ?∆¶??∆¨?∆¶]\n" + gCreator + "\n\n[∆§∆¶???‘º ?∆¶?∆≤∆§]\nhttp://dl.profile.line.naver.jp/" + ginfo.pictureStatus + "\n\nM?M??∆¶:" + str(len(ginfo.members)) + "Person\n????∆¨?:" + sinvitee + "Person\n????∆¨?∆¨??? ∆≤∆¶‘º:" + u + "?∆¨'∆ß ∆¨H? ??∆ß???.")
                    else:
                        cl.sendText(msg.to,"[?çÂ?]\n" + str(ginfo.name) + "\n[gid]\n" + msg.to + "\n[Â∞èÁ??Ñ‰??êËÄÖ]\n" + gCreator + "\n[Â∞èÁ??æÊ?]\nhttp://dl.profile.line.naver.jp/" + ginfo.pictureStatus)
                else:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"It can't be used besides the group??")
                    else:
...
#--------------------------------------------------------
                ...
                try:
                    ...
                    if strnum == "off":
                        ...
                        if wait["lang"] == "JP":
                            cl.sendText(msg.to,"Invitation refusal was turned off?Ç\non, please designate and send the number of people.")
                        else:
                ...
                if wait["leaveRoom"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"∆¨∆≤∆¶??? ?????")
            elif msg.text in ["?±Ê?:?™„É≥","?±Ê?Ôºö„Ç™??","Auto share:on"]:
                if wait["timeline"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"?‘º∆¶???∆≥ ??")
                    else:
            ...
            elif msg.text in ["?±Ê?:?™„?","?±Ê?Ôºö„Ç™??","Auto share:off"]:
                ...
                if wait["likeOn"] == True:
                    ...
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Already??")
            elif msg.text in ["?Ñ„???:?™„?","Auto like:off"]:
            ...
            elif msg.text in ["Add confirmasi","?™Â?ËøΩÂ??èÂÄôË?Á¢∫Ë?"]:
                if wait["lang"] == "JP":
                    cl.sendText(msg.to,".automatic message is established as follows?Ç\n\n" + wait["message"])
                else:
                    cl.sendText(msg.to,"One  of weeds on the surface below the self- additional breath image?Ç\n\n" + wait["message"])
            elif msg.text in ["CHANGE","Ë®ÄË™ûË???"]:
                if wait["lang"] =="JP":
                    wait["lang"] = "TW"
                    cl.sendText(msg.to,"??∆≤?∆¨∆¶∆≥ ‘º???∆≤??? ?∆≤∆¶??? ? ?H??????")
                else:
                    wait["lang"] = "JP"
                ...
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"?∆¨ ???'∆¨ ?? ∆≤∆ß?? ??∆ß???∆ß ∆¨H? ?∆¶?∆≤∆§.??")
                    else:
                        cl.sendText(msg.to,"?M∆§?∆ß∆ß??‘º? ∆≤∆ß? ??∆ß???∆ß ∆¨H? ?∆¶?∆≤∆§. ")
                ...
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"??? ??∆¨ ?? ∆≤∆ß?? ?∆≤∆¨∆ß??? ∆¨H? ?∆¶?∆≤∆§")
                    else:
                        cl.sendText(msg.to,"??∆¨ ??∆¶ ∆≤∆ß? ‘º?∆ß∆ß ∆¨H?? ?∆¶?∆≤∆§")
            elif msg.text in ["cb"]:
                wait["wblack"] = True
...
#-----------------------------------------------------------
                ...
                for g in gs.members:
                    if _nametarget == g.displayName:
                       ...
                       kk.sendText(g.mid,"Spam  ??")
                       ks.sendText(g.mid,"Spam  ??")
                       kc.sendText(g.mid,"H?H?H? ?? ∆ß∆§?M  ??")
                       ka.sendText(g.mid,"H?H?H? ?? ∆ß∆§?M  ??")
                       ks.sendText(g.mid,"H?H?H? ?? ∆ß∆§?M  ??")
                       cl.sendText(msg.to, "???? ∆ß∆§?M  ??")
                       print "Done spam" 
#-----------------------------------------------------------
            elif msg.text in ["PING","Ping","ping"]:
                ki.sendText(msg.to,"∆§??? ???Ñªdouble thumbs up?øø????Har Har?øø")
                kk.sendText(msg.to,"∆§??? ???Ñªdouble thumbs up?øø????Har Har?øø")
                ks.sendText(msg.to,"∆§??? ???Ñªdouble thumbs up?øø????Har Har?øø")
                kc.sendText(msg.to,"∆§??? ???Ñªdouble thumbs upÔøΩÙ?1?7???Har HarÔø?1?7")
                ka.sendText(msg.to,"∆§??? ???Ñªdouble thumbs upÔøΩÙ?1?7???Har HarÔø?1?7")
#----------------------------------------------------------
            elif msg.text in ["Respon","respon","responsename"]:
                cl.sendText(msg.to,"???∆¨?∆¶ ??∆≤? ??∆¨ ‘º??? ?.2 ????")
                ki.sendText(msg.to,"??∆¨ ‘º??? 1")
                kk.sendText(msg.to,"??∆¨ ‘º??? 2")
                ks.sendText(msg.to,"??∆¨ ‘º??? 3")	
                kc.sendText(msg.to,"??∆¨ ‘º??? 4")
                ka.sendText(msg.to,"??∆¨ ‘º??? 5")
#----------------------------------------------------------
            elif msg.text == "‡∏°‡∏≠‡∏?1?7":
              if msg.from_ in admin:
                cl.sendText(msg.to, "∆ß?∆¨ ∆¨H? ‘º?∆ß∆¨∆ß???∆ß' ∆§???∆¨(ÔΩ?1?7?ªœâ„Éª¬¥)")
                try:
		  ...
		  if msg.to in wait2['readPoint']:
	            ...
	            cl.sendText(msg.to, " %s\n\n\nPeople who have ignored reads\n(ÔΩ?1?7?ªœâ„Éª¬¥)\n%s\n\nThese anu anu uesrs have seen at the lastseen point(ÔΩ?1?7?ªœâ„Éª¬¥)\n[%s]"  % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
	          else:
	            cl.sendText(msg.to, "Sider ga bisa di read cek setpoint dulu bego tinggal ketik\nSetlastpoint\nkalo mau liat sider ketik\nViewlastseen")
#-----------------------------------------------------------speed
            elif msg.text in ["Ban"]:
                wait["wblacklist"] = True
                cl.sendText(msg.to,"∆§‘º??∆ß? ∆ß??? ∆¨H? ????∆≤?∆¨ ∆¶???∆ß∆¨?∆¶?? ??∆¨H ? ?‘º???‘º?∆ß∆¨??1?7")
            elif msg.text in ["Unban"]:
                wait["dblacklist"] = True
                cl.sendText(msg.to,"∆§‘º??∆ß? ∆ß??? ∆¨H? ????∆≤?∆¨ ∆¶???∆ß∆¨?∆¶?? ??∆¨H ? ?‘º???‘º?∆ß∆¨??1?7")
            elif msg.text in ["Banlist"]:
                if wait["blacklist"] == {}:
                    cl.sendText(msg.to,"∆¨H?∆¶? ?∆ß?'∆¨ ? ∆§?∆¶∆ß?? M??? ? ?‘º???‘º?∆ß∆¨.??1?7")
                else:
                    cl.sendText(msg.to,"??‘º?? ?∆ß ? ?‘º???‘º?∆ß∆¨??1?7")
                    mc = ""
